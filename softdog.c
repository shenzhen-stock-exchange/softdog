...
#define TRY_MMAP_LENGTH 1024000
/* function prototypes */
...
#endif					
					sleep(SLEEP_DELAY/3);			/* this should not fail */	
					continue;						/* this should not fail */
				}		
				/* SZSE rickyli 20150827 ---end---*/
...
{
#ifdef DEBUG    			  
			fprintf(stderr, "trying to mmap\n");
#endif
		void *p;

		p = mmap (0, TRY_MMAP_LENGTH, PROT_READ, MAP_SHARED, fd, 0);
        ...
        if (close (fd) == −1) {
                perror ("close mmaped memory zone failed");
                return 1;
        }

        for (len = 0; len < TRY_MMAP_LENGTH; len++)
                putchar (p[len]);

        if (munmap (p, TRY_MMAP_LENGTH) == −1) {
